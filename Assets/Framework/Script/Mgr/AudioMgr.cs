using Newtonsoft.Json;
using System;
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.Networking;
using System.Collections;

public class AudioMgr : MonoSingleton<AudioMgr>
{
    public readonly string audioVolumeConfigPath = Application.streamingAssetsPath + "/AudioVolumeConfig.json";
    /// <summary>
    ///  音量配置
    /// </summary>
    AudioVolumeData volumeData = new AudioVolumeData();
    AudioSource soundAudio;
    AudioSource musicAudio;
    readonly string soundFilePath = "Audio/sound/";
    readonly string musicFilePath = "Audio/music/";
    public float soundVolume { get { return soundAudio.volume; } set { soundAudio.volume = value; } }
    public float musicVolume { get { return musicAudio.volume; } set { musicAudio.volume = value; RefreshAllMusicVolume(); } }
    //同时存在的背景音乐播放器
    public Dictionary<string, OverlayMusicData> newMusicDict = new Dictionary<string, OverlayMusicData>();

    private void Awake()
    {
        StartCoroutine(LoadConfig());
        soundAudio = new GameObject("soundAudio").AddComponent<AudioSource>();
        soundAudio.transform.SetParent(transform);
        musicAudio = new GameObject("musicAudio").AddComponent<AudioSource>();
        musicAudio.loop = true;
        musicAudio.transform.SetParent(transform);
    }

    private IEnumerator LoadConfig()
    {
        string path = audioVolumeConfigPath;
#if UNITY_IOS || UNITY_EDITOR_OSX
        path = "file://" + path;
#endif
        using (UnityWebRequest unityWebRequest = UnityWebRequest.Get(path))
        {
            yield return unityWebRequest.SendWebRequest();
            if (unityWebRequest.result == UnityWebRequest.Result.Success)
            {
                string config = unityWebRequest.downloadHandler.text;
                volumeData = JsonConvert.DeserializeObject<AudioVolumeData>(config);
                Utility.Dump(volumeData, "音频配置读取成功");
            }
            else
            {
                Debug.LogError("音频配置读取失败" + unityWebRequest.error);
            }
        }
    }

    public float GetVolume(string _audioName, Type _type)
    {
        float val = 1f;
        if (_type == typeof(AudioSound))
        {
            if (DataMgr.Ins.settingData.soundVolume == 0)
            {
                return 0;
            }
            if (volumeData.soundVolume.ContainsKey(_audioName))
            {
                val = volumeData.soundVolume[_audioName];
            }
        }
        else if (_type == typeof(AudioMusic))
        {
            if (DataMgr.Ins.settingData.musicVolume == 0)
            {
                return 0;
            }
            if (volumeData.musicVolume.ContainsKey(_audioName))
            {
                val = volumeData.musicVolume[_audioName];
            }
        }
        return val;
    }

    /// <summary>
    /// 播放音乐
    /// </summary>
    /// <param name="_music"></param>
    public void PlayMusic(AudioMusic _music, AudioMusic _defaultMusic = AudioMusic.None)
    {
        PlayMusic(_music.ToString(), _defaultMusic);
    }

    public void PlayMusic(string _music, AudioMusic _defaultMusic = AudioMusic.None)
    {
        Type t = typeof(AudioMusic);
        musicAudio.clip = GetAudioClip(_music, t, _defaultMusic.ToString());
        float volume = GetVolume(_music.ToString(), t);
        musicAudio.volume = volume;
        musicAudio.Play();
    }

    /// <summary>
    /// 播放音效
    /// </summary>
    public void PlaySound(AudioSound _sound)
    {
        if (_sound == AudioSound.None)
        {
            return;
        }
        Type t = typeof(AudioSound);
        AudioClip audioClip = GetAudioClip(_sound.ToString(), t);
        if (audioClip != null)
        {
            soundAudio.PlayOneShot(audioClip, GetVolume(_sound.ToString(), typeof(AudioSound)));
        }
    }

    public AudioClip GetAudioClip(string _sound, Type _type, string _defaultAudio = "")
    {
        string path = _type == typeof(AudioSound) ? soundFilePath : musicFilePath;
        AudioClip audioClip = Resources.Load<AudioClip>(path + _sound);
        if (!string.IsNullOrEmpty(_defaultAudio) && audioClip == null)
        {
            audioClip = Resources.Load<AudioClip>(path + _defaultAudio);
        }
        return audioClip;
    }

    public void AddOverlayMusic(AudioMusic _audioMusic, bool _loop = true, AudioMusic _defaultMusic = AudioMusic.None)
    {
        AddOverlayMusic(_audioMusic, _loop, _defaultMusic);
    }
    /// <summary>
    /// 创建一个新的music可以叠加能主动停止
    /// </summary>
    public void AddOverlayMusic(string _audioMusic, bool _loop = true, AudioMusic _defaultMusic = AudioMusic.None)
    {
        if (newMusicDict.ContainsKey(_audioMusic))
        {
            return;
        }
        OverlayMusicData overlayData = new OverlayMusicData();
        AudioSource audio = gameObject.AddComponent<AudioSource>();
        overlayData.audioSource = audio;
        Type t = typeof(AudioMusic);
        audio.clip = GetAudioClip(_audioMusic, t, _defaultMusic.ToString());
        audio.loop = _loop;
        audio.Play();
        float volume = GetVolume(_audioMusic.ToString(), t);
        audio.volume = volume;
        if (!_loop)
        {
            overlayData.timer = Timer.Ins.SetTimeOut(() =>
            {
                RemoveOverlayMusic(_audioMusic);
            }, audio.clip.length);
        }
        newMusicDict[_audioMusic] = overlayData;
    }

    public void RemoveOverlayMusic(AudioMusic _audioMusic)
    {
        RemoveOverlayMusic(_audioMusic.ToString());
    }
    /// <summary>
    /// 停止一个新的music播放
    /// </summary>
    public void RemoveOverlayMusic(string _audioMusic)
    {
        if (!newMusicDict.ContainsKey(_audioMusic))
        {
            return;
        }
        OverlayMusicData overlayData = newMusicDict[_audioMusic];
        overlayData.timer?.Remove();
        Destroy(overlayData.audioSource);
        newMusicDict.Remove(_audioMusic);
    }

    /// <summary>
    /// 叠加背景音
    /// </summary>
    public class OverlayMusicData
    {
        public AudioSource audioSource;
        public TimerHandler timer;
    }

    /// <summary>
    /// 设置所有播放器的开关
    /// </summary>
    /// <param name="_play"></param>
    public void SetAllMusicEnable(bool _play)
    {
        if (_play)
        {
            foreach (var item in newMusicDict)
            {
                item.Value.audioSource?.Play();
            }
            musicAudio.Play();
        }
        else
        {
            foreach (var item in newMusicDict)
            {
                item.Value.audioSource?.Pause();
            }
            musicAudio.Pause();
        }
    }

    /// <summary>
    /// 设置音乐音量
    /// </summary>
    public void RefreshAllMusicVolume()
    {
        foreach (var item in newMusicDict)
        {
            item.Value.audioSource.volume = musicVolume;
        }
        musicAudio.volume = musicVolume;
    }
}