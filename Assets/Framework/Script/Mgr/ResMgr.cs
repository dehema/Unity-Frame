...
public class ResMgr : MonoSingleton<ResMgr>
{
    ...
    {
        //获取资源图片物体
        GameObject resGp = GetResImage(_resType).gameObject;
        //收集金币动画
        AnimationController.GoldMoveBest(resGp, 10, _startTf, resGp.transform, _cb);
        GameObject FX_AddCard = Instantiate(Resources.Load<GameObject>("VFX/FX_HealBig"), _startTf);
    }
    ...
    public void AddRes(ResType _resType, float _num)
    {
        DBObject dbObject = GetResFeild(_resType);
        if (dbObject != null)
        {
            DBFloat dBFloat = (DBFloat)dbObject;
            dBFloat.Value += _num;
    ...
    public DBObject GetResFeild(ResType _resType)
    {
        switch (_resType)
        {
            ...
            case ResType.cash:
                return DataMgr.Ins.playerData.cash;
            case ResType.amazon:
                return DataMgr.Ins.playerData.amazon;
        }
        return null;
    }
