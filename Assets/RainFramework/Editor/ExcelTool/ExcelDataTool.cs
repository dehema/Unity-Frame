...
namespace Rain.Core.Editor
{
    ...
    {
        public const string CODE_NAMESPACE = "Rain.F8ExcelDataClass"; //由表生成的数据类型均在此命名空间内
        ...
        public const string DataManagerFolder = "/RainFramework/ConfigData/F8DataManager"; //Data代码路径
        ...
        public const string DLLFolder = "/RainFramework/ConfigData"; //存放dll目录
    ...
    ""noEngineReferences"": false
            ...
            RLog.LogConfig("创建.asmdef文件 " + Application.dataPath + DLLFolder + "/<color=#FF9E59>" + CODE_NAMESPACE + ".asmdef" + "</color>");
        }
        ...
        public static void LoadAllExcelData()
        {
            if (F8EditorPrefs.GetString("ExcelPath", default).IsNullOrEmpty())
            {
                ...
                RLog.LogConfig("首次启动，设置Excel存放目录：" + tempExcelPath + " （如要更改请到----上方菜单栏->开发工具->设置Excel存放目录）");
            }
            ...
            if (files == null || files.Length == 0)
            {
                ...
                files = Directory.GetFiles(INPUT_PATH, "*.*", SearchOption.AllDirectories)
                    .Where(s => s.EndsWith(".xls") || s.EndsWith(".xlsx")).ToArray();
                RLog.LogError("暂无可以导入的数据表！自动为你创建：【DemoWorkSheet.xlsx / Localization.xlsx】两个表格！" + lastExcelPath + " 目录");
            }
            ...
            RLog.LogConfig("清空目录：" + F8ExcelDataClassPath);
            ...
            RLog.LogConfig("清空目录：" + F8DataManagerPath);
            ...
            RLog.LogConfig("删除文件：" + F8ExcelDataClassPathDLL);
        ...
        private static void AllScriptsReloaded()
        {
            ...
            RLog.LogConfig("<color=#FF9E59>导表后脚本编译完成!</color>");
            ...
            RLog.LogConfig("<color=yellow>导表成功!</color>");
        ...
        private static Assembly CompileCode(string[] scripts)
        {
            ...
            if (cr.Errors.HasErrors)
            {
                ...
                {
                    RLog.LogError("编译dll出错：" + err.ErrorText);
                }

                throw new Exception("编译dll出错！请检查配置表格式！");
            }

            RLog.LogConfig("已编译 " + path + "/<color=#FFFF00>" + CODE_NAMESPACE + ".dll</color>");
        ...
        public static void GenerateCodeFiles(Dictionary<string, ScriptGenerator> codeList)
        {
            ...
            foreach (var kvp in codeList)
            {
                ...
                RLog.LogConfig($"已生成代码 " + path + "/<color=#FF9E59>" + kvp.Key + ".cs</color>");
            }
        }
...
// #if UNITY_WEBGL
			try
            {
                ...
                // 记录日志
                RLog.LogConfig("已序列化 " + BinDataPath + "/<color=#FFFF00>" + container.GetType().Name + ".json</color>");
            }
            catch (Exception e)
            {
                RLog.LogError($"序列化失败: {e.Message}");
            }
// #else
