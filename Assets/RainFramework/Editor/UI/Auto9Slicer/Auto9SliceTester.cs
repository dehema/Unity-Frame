...
namespace Rain.Core.Editor
{
    public class Auto9SliceTester : UnityEditor.Editor
    {
        ...
        [MenuItem("Assets/【RainUI界面管理功能】/（图片自动切割九宫格）", false, 1022)]
        public static void Auto9Slice()
        {
            ...
            foreach (var target in texture2DList)
            {
                ...
                if (importer is TextureImporter textureImporter)
                {
                    ...
                    {
                        RLog.Log($"已设置九宫格，跳过 {Path.GetFileName(target)}");
                        continue;
                    }
                    ...
                    RLog.Log($"图片九宫格切割完成！{Path.GetFileName(target)} = {textureImporter.spriteBorder}");
                }
            }
