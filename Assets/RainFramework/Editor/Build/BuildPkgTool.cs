...
namespace Rain.Core.Editor
{
    public class BuildPkgTool : ScriptableObject
    {
        ...
        public static void BuildUpdate()
        {
            ...
            {
                RLog.LogError("请先构建一个游戏版本，再构建热更新文件！~");
                return;
            }
            ...
            RLog.LogVersion("构建热更新包版本成功！版本：" + toVersion);
        ...
        public static void Build()
        {
            ...
            if (enableFullPackage)
            {
                ...
                {
                    RLog.LogError($"导出失败了，检查一下 Unity 内置的 Build Settings 导出的路径是否存在，Unity 没有给我清理缓存！: {buildReport.summary.result}");
                }

                RLog.LogVersion("游戏全量包打包成功! " + locationPathName);
            }
            ...
            if (enableOptionalPackage)
            {
                ...
                foreach (var package in packages)
                {
                    ...
                    FileTools.SafeDeleteDir(packagePath);
                    RLog.LogVersion("分包输出目录：" + zipName + " ，手动上传至CDN资源服务器。");
                }
                ...
                RLog.LogVersion("游戏分包打包成功! " + locationPathName);
            }
            ...
            if (enableNullPackage)
            {
                ...
                if (buildReport.summary.result != BuildResult.Succeeded)
                {
                    RLog.LogError($"导出失败了，检查一下 Unity 内置的 Build Settings 导出的路径是否存在，并使用 Unity 内置打包工具打包一次，或 Unity 没有给我清理缓存，尝试使用 Clean 打包模式！: {buildReport.summary.result}");
                }
                ...
                FileTools.SafeDeleteDir(toPath);
                RLog.LogVersion("游戏空包打包成功! " + locationPathName);
            }
        ...
        public static void WriteGameVersion()
        {
            ...
            // 写入到文件
            string gameVersionResourcesPath = Application.dataPath + "/RainFramework/AssetMap/Resources/" + nameof(GameVersion) + ".json";
            // 序列化对象
            ...
            RLog.LogVersion("写入游戏版本： " + gameVersion.Version);
            UnityEditor.AssetDatabase.Refresh();
        }
        ...
        private static BuildTargetGroup GetBuildTargetGroup(BuildTarget target)
        {
            ...
            {
                RLog.LogError($"Could not find BuildTargetGroup for BuildTarget {target}");
                return default;
            }
        }
