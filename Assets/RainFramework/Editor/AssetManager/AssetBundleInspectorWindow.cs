...
namespace Rain.Core.Editor
{
    public class AssetBundleInspectorWindow : EditorWindow
    {
        ...
        {
            // 获取Resources加载器数据
            resourceLoaders = ResourcesManager.Ins.GetResourceLoaders();
        ...
        private void DrawResourceDetails(ResourcesLoader loader)
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                {
                    RLog.Log($"卸载Resources资源 {loader.resourcePath}");
                    loader.Clear();
                }

                if (!loader.IsLoadFinished && GUILayout.Button("重新加载", EditorStyles.miniButton))
                {
                    RLog.Log($"加载Resources资源 {loader.resourcePath}");
                    loader.Load();
                }
            }
            EditorGUILayout.EndHorizontal();
        ...
        {
            assetBundleLoaders = AssetBundleManager.Ins.GetAssetBundleLoaders();
        ...
        private void DrawAssetBundleDetails(AssetBundleLoader loader)
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                {
                    RLog.Log($"重新加载AB包 {loader.abName} 中的资源");
                ...
                {
                    RLog.Log($"完全卸载AB包 {loader.abName}");
                    loader.Unload(true);
                }
            }
            EditorGUILayout.EndHorizontal();
