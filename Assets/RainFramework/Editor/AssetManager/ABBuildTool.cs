...
namespace Rain.Core.Editor
{
    public class ABBuildTool : ScriptableObject
    {
        ...
        public static void BuildAllAB()
        {
            ...
            RLog.LogAsset("自动设置AssetBundleName（AB名为空时）");
            ...
            RLog.LogAsset("打包AssetBundle：" + URLSetting.GetAssetBundlesOutPath() + "  当前打包平台：" + EditorUserBuildSettings.activeBuildTarget);
            ...
            foreach (string ab in abPaths)
            {
                if (!assetPaths.Contains(ab) && !AssetPathsContainsDiscrepantAssetBundle(assetPaths, ab))
                {
                    ...
                    else if (Directory.Exists(abpath))
                    {
                        ...
                        {
                            RLog.LogAsset("删除多余AB文件夹：" + abpath);
                    ...
                    {
                        RLog.LogAsset("AB文件路径不存在，已经被删除了：" + abpath);
                    }
                }
            }
            RLog.LogAsset("删除全部的.manifest文件");
        ...
        public static string SetAssetBundleName(string path)
        {
            ...
            if (!ai.assetBundleName.Equals(bundleName))
            {
                ...
                else if (DiscrepantAssetPathMapping != null)
                {
                    ...
                    {
                        RLog.LogError("资产父路径和AB名不相等，检查是否迁移过文件路径，并清理AB名：" + ai.assetBundleName + " -> " + bundleName + "，资产路径：" + path);
                    }
        ...
        public static void GenerateAssetNames(bool isWrite = false)
        {
            ...
            if (Directory.Exists(URLSetting.GetAssetBundlesFolder()))
            {
                ...
                foreach (string _filePath in allAssetBundlesPaths)
                {
                    ...
                    if (File.Exists(filePath)) // 文件
                    {
                        ...
                        if (tempNames.Contains(fileNameWithoutExtension.ToLower()))
                        {
                            ...
                            RLog.Log("AB资源名称重复（大小写不敏感）：" + filePath + "，增加唯一识别ID后为：" + fileNameWithoutExtension);
                        }
                        ...
                        {
                            RLog.LogError("AssetBundle名中不推荐含有中文： " + filePath);
                        }
                    }
                    else if (Directory.Exists(filePath)) // 文件夹
                    {
                        ...
                        if (tempNames.Contains(fileNameWithoutExtension))
                        {
                            string id = Util.Encryption.MD5Encrypt(assetPath);
                            fileNameWithoutExtension += id;
                            RLog.Log("AB文件夹名称重复（大小写不敏感）：" + filePath + "，增加唯一识别ID后为：" + fileNameWithoutExtension);
                        }
                        ...
                        if (filePath.IsContainChinese())
                        {
                            RLog.LogError("AssetBundle文件夹中不推荐含有中文： " + filePath);
                        }
                    }
                }
                ...
                if (isWrite)
                {
                    ...
                    {
                        RLog.LogError("总AssetBundleManifest和其他资产名重复，请检查资产：" + URLSetting.GetPlatformName());
                    }
                    else
                    {
        ...
        private static void WriteAssetNames()
        {
            ...
            string AssetBundleMapPath = Application.dataPath + "/RainFramework/AssetMap/Resources/" + nameof(AssetBundleMap) + ".json";
            ...
            RLog.LogAsset("写入AssetBundles资产数据 生成：" + AssetBundleMapPath);
        }
                ...
                foreach (string _filePath in allPaths)
                {
                    ...
                    if (File.Exists(filePath)) // 文件
                    {
                        ...
                        if (tempNames.Contains(fileNameWithoutExtension))
                        {
                            string id = Util.Encryption.MD5Encrypt(assetPath);
                            fileNameWithoutExtension += id;
                            RLog.Log("Resources资源名称重复（大小写不敏感）：" + filePath + "，增加唯一识别ID后为：" + fileNameWithoutExtension);
                        }
                    ...
                    else if (Directory.Exists(filePath)) // 文件夹
                    {
                        ...
                        if (tempNames.Contains(fileNameWithoutExtension))
                        {
                            string id = Util.Encryption.MD5Encrypt(assetPath);
                            fileNameWithoutExtension += id;
                            RLog.Log("Resources文件夹名称重复（大小写不敏感）：" + filePath + "，增加唯一识别ID后为：" + fileNameWithoutExtension);
                        }
        ...
        private static void WriteResourceNames()
        {
            ...
            string ResourceMapPath = Application.dataPath + "/RainFramework/AssetMap/Resources/" + nameof(ResourceMap) + ".json";
            FileTools.CheckFileAndCreateDirWhenNeeded(ResourceMapPath);
            FileTools.SafeWriteAllText(ResourceMapPath, Util.LitJson.ToJson(resourceMapping));
            AssetDatabase.Refresh();

            RLog.LogAsset("写入Resources资产数据 生成：" + ResourceMapPath);
        }
        ...
        private static string ParseManifestFile(string manifestPath)
        {
            ...
            {
                RLog.LogError($"Manifest 文件不存在: {manifestPath}");
                return null;
            }
            ...
            RLog.LogError("Manifest 文件内不存在 Hash:");
            return null;
        }
