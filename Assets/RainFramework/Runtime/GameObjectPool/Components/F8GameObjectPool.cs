...
namespace Rain.Core
{
    ...
    public sealed class F8GameObjectPool : MonoBehaviour
    {
        ...
        public void Init(GameObject prefab)
        {
            ...
            if (prefab == null)
            {
                RLog.LogError("您正在尝试使用空预制体初始化此池！", this);
                return;
            }

            if (_hasPreloadedGameObjects && _prefab != prefab)
            {
                RLog.LogError("此池已预加载游戏对象，而您正在尝试使用另一个预制体初始化此池！" +
        ...
        public void PopulatePool(int count)
        {
            ...
            if (_isSetup == false)
            {
                RLog.LogError($"池 '{name}' 未设置！", this);
                return;
            }

            if (Application.isPlaying == false)
            {
                RLog.LogError($"在应用程序未运行时，您正在尝试填充池 '{name}'！", this);
                return;
            }

            if (count < 0)
            {
                RLog.LogError("填充数量不能小于零！", this);
                return;
            }
#endif
        ...
        public void SetCapacity(int capacity)
        {
            ...
            if (capacity < 0)
            {
                RLog.LogError($"池 '{name}' 的容量不能小于零！", this);
                return;
            }

            if (capacity < _allClonesCount)
            {
                RLog.LogError($"池 '{name}' 的容量不能小于所有克隆的数量！", this);
                return;
            }

            if (_hasPreloadedGameObjects && _capacity < _gameObjectsToPreload.Count)
            {
                RLog.LogError($"池 '{name}' 的容量不能小于预加载克隆的数量！", this);
                return;
            }

            if (_sendWarnings && capacity == 0)
            {
                RLog.LogEntity($"池 '{name}' 的容量为零。");
            }
#endif
            _capacity = capacity;
        ...
        internal bool TrySetup(GameObject prefab)
        {
            ...
            if (Application.isPlaying == false)
            {
                RLog.LogError("应用程序未运行时，无法设置池！", this);
                return false;
            }
        ...
        internal void Get(out GettingPoolableArguments arguments)
        {
            if (_despawnedPoolables._count <= 0)
            {
                if (_allClonesCount >= _capacity)
                {
                    ...
                    {
#if DEBUG
                        RLog.LogError("已达到容量上限！无法生成新的克隆！", this);
#endif
                        arguments = new GettingPoolableArguments(null, true);
                        return;
                    }
        ...
        internal void Release(Poolable poolable)
        {
            if (poolable._status == PoolableStatus.Despawned)
            {
                ...
                if (_sendWarnings)
                {
                    RLog.LogEntity($"池对象 '{poolable._gameObject}' 已经取消生成！", poolable._gameObject);
                }
#endif
                return;
            }
        ...
        private bool CheckForPrefab(GameObject gameObjectToCheck)
        {
            ...
            {
                RLog.LogError("您不能将场景中的游戏对象设置为预制体！", this);
                _prefab = null;
                return false;
            }
            ...
            {
                RLog.LogError($"'{gameObjectToCheck}' 不是一个预制体！", this);
                _prefab = null;
                return false;
            }
        ...
        private void ClampCapacity()
        {
            if (_hasPreloadedGameObjects && _capacity < _gameObjectsToPreload.Count)
            {
                RLog.LogError("容量不能小于预加载克隆的数量！", this);
            ...
            if (_despawnedPoolables != null)
            {
                ...
                {
                    RLog.LogError("容量不能小于所有克隆的数量！", this);
        ...
        private void CheckPreloadedClonesForErrors()
        {
            if (_hasPreloadedGameObjects)
            {
                ...
                if (_prefab == null)
                {
                    RLog.LogError("此池中已预加载游戏对象，但现在预制体为空！" +
                ...
                for (int i = 0; i < _gameObjectsToPreload.Count; i++)
                {
                    ...
                    {
                        RLog.LogError("您预加载的游戏对象与预制体不匹配。" +
                                       "清除此池或设置正确的预制体。", this);
                        return;
                    }
                }
        ...
        private bool CanPreloadGameObject()
        {
            if (_prefab == null)
            {
                RLog.LogError($"池 '{name}' 的预制体为空！", this);
                return false;
            }
            ...
            if (_gameObjectsToPreload.Count >= _capacity || _allClonesCount >= _capacity)
            {
                if (_sendWarnings)
                {
                    RLog.LogEntity("已达到容量上限！无法预加载更多游戏对象！", this);
                }
        ...
        private void SetupPreloadedClones()
        {
            for (var i = 0; i < _gameObjectsToPreload.Count; i++)
            {
                ...
                if (clone == null)
                {
                    RLog.LogError($"其中一个预加载的游戏对象已被销毁！在应用程序未运行时，从组件上下文菜单清除 '{name}' 池 " +
                                   "以解决此问题！", this);
                    continue;
                }
#endif
