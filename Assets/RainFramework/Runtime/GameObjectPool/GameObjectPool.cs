    ...
    public class GameObjectPool : ModuleSingleton<GameObjectPool>, IModule
    {
        ...
        public void InstallPools(PoolsPreset poolsPreset)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (prefab == null)
                {
                    RLog.LogError($"名称为{nameof(PoolsPreset)}的'{poolsPreset}'预设中有一个或多个空的预制体!",
                        poolsPreset);
...
#if DEBUG
                    RLog.LogError($"您正在尝试通过{nameof(PoolsPreset)} '{poolsPreset}'安装的池 '{pool}' 已经存在!",
                        pool);
#endif
                }
            }
        ...
        public GameObject Spawn(string prefabName)
        {
            ...
            if (!prefab)
            {
                RLog.LogError("对象池未创建，通过名称生成对象失败。");
                return null;
            }
...
#if DEBUG
            if (hasPool == false)
                RLog.LogError($"克隆 '{clone}' 未找到对应的池!",
                    clone);
#endif
            return pool;
        }
...
#if DEBUG
            if (hasPool == false)
                RLog.LogError($"未通过预制体 '{prefab}' 找到池!",
                    prefab);
#endif
            return pool;
        }
...
#if DEBUG
            RLog.LogError($"未通过预制体名称 '{prefabName}' 找到池!", prefabName);
#endif
            return null;
        }
        ...
        public void DestroyAllPools(bool immediately = false)
        {
            ...
            {
#if DEBUG
                RLog.LogError("在应用程序退出时，您正在尝试销毁所有池！");
#endif
                return;
            }
        ...
        internal void RegisterPool(F8GameObjectPool pool)
        {
            ...
            {
                RLog.LogError($"您正在尝试注册另一个使用相同预制体 '{pool._prefab}' 的池 '{pool.name}'!",
                    pool);
            }
#endif
        }
        ...
        internal void RegisterPersistentPool(F8GameObjectPool pool)
        {
            if (pool._dontDestroyOnLoad)
            {
                ...
                else
                {
                    ...
                    {
                        RLog.LogEntity($"您正在尝试注册持久池 '{pool.name}' 两次！",
                            pool);
                    }
                }
#endif
        ...
        internal void DespawnImmediate(Poolable poolable)
        {
            ...
            else
            {
                ...
                if (Warnings)
                {
                    RLog.LogEntity($"可池化对象 '{poolable._gameObject}' 尚未设置并将被销毁！",
...
#if DEBUG
                        RLog.LogEntity($"<{nameof(F8PoolGlobal)}> 实例已自动创建。也可以手动添加以修改默认参数。");
#endif
                    }
                }
        ...
        private bool TryFindF8PoolInstanceAsSingle(out F8PoolGlobal f8Pool)
        {
            ...
            if (length > 0)
            {
                ...
                if (length > 1)
                {
                    ...
                    RLog.LogError($"场景中 {nameof(F8PoolGlobal)} 实例的数量大于一个！");
                }
#endif
                f8Pool = instances[0];
                return true;
            ...
            Transform parent, bool worldPositionStays, out bool haveToGetComponent)
        {
            ...
            {
#if DEBUG
                RLog.LogError($"在应用程序退出时，您正在尝试生成预制体 '{prefab}'！", prefab);
#endif
            ...
            if (s_checkClonesForNull)
            {
                ...
                {
                    RLog.LogError("您正在尝试生成一个已经在没有 {nameof(GameObjectPool)} 的情况下被销毁的克隆！预制体: '{prefab}'", pool);
                }
            }
#endif
        ...
        private void DefaultDespawn(GameObject gameObject, float delay = 0f)
        {
            ...
            {
#if DEBUG
                RLog.LogError($"在应用程序退出时，您正在尝试取消生成 '{gameObject}'！", gameObject);
#endif
                return;
            }
            ...
            if (ClonesMap.TryGetValue(gameObject, out Poolable poolable))
            {
                if (poolable._status == PoolableStatus.Despawned)
                {
                    ...
                    {
                        RLog.LogEntity("您要取消生成的游戏对象已经被取消生成！", gameObject);
                    }
#endif
                    return;
                }
            ...
            else
            {
                ...
                if (Warnings)
                {
                    RLog.LogEntity($"'{gameObject}' 未使用 {nameof(GameObjectPool)}（或池已销毁）生成，并将被销毁！", gameObject);
                }
#endif
                ...
                if (HasDespawnRequest(poolable, out int index))
                {
                    ...
                    switch (reaction)
                    {
                        ...
                        case ReactionOnRepeatedDelayedDespawn.ThrowException:
                            ...
                            {
                                RLog.LogError("延迟取消生成请求已经存在于该克隆！", poolable._gameObject);
                            }
#endif
                            break;
                    }
        ...
        private bool CanPerformPoolAction()
        {
            if (s_isApplicationQuitting)
            {
                ...
                {
                    RLog.LogError($"<{nameof(F8PoolGlobal)}> 实例为空！");
                }
#endif
                return false;
            }
            ...
            if (s_hasTheF8PoolInitialized == false)
            {
                ...
                {
                    RLog.LogError("在应用程序未运行时，您正在尝试执行生成或取消生成操作！");
                }
#endif
                InitializeTheF8Pool();
            }
        ...
        private void CheckPoolableForLightweightTransformSetup(F8GameObjectPool pool, Poolable poolable)
        {
            ...
            {
                RLog.LogError($"池及其父物体在 F8 池 '{nameof(F8PoolMode.Performance)}' 模式下必须具有相同的缩放，即 'Vector3.one'！",
                    poolable._pool);
        ...
        private void DestroyPoolableWithGameObject(GameObject clone, bool immediately)
        {
            if (ClonesMap.TryGetValue(clone, out Poolable poolable))
            {
                ...
                {
                    RLog.LogError($"克隆 '{clone}' 尚未设置！", clone);
                }
#endif
            }
            else
            {
#if DEBUG
                RLog.LogEntity($"克隆 '{clone}' 并非由 {nameof(GameObjectPool)} 生成！", clone);
#endif
