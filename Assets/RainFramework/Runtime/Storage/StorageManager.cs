...
namespace Rain.Core
{
    public class StorageManager : ModuleSingleton<StorageManager>, IModule
    {
        ...
        private byte[] GetRawBytes(string key, bool user = false)
        {
            ...
            {
                RLog.LogError($"Invalid Base64 format for key: {key}");
                return null;
            }
            catch (CryptographicException)
            {
                RLog.LogError($"Cryptographic error occurred while decrypting key: {key}");
                return null;
            }
            catch (Exception ex)
            {
                RLog.LogError($"Unexpected error occurred while getting raw bytes for key: {key}. Error: {ex.Message}");
                return null;
            }
        }
